defmodule Squares do
  ...
  # def sum_of_squares(number), do: 1..number |> Enum.map(&(&1 ** 2)) |> Enum.sum()
  def sum_of_squares(number), do: div(number * (number + 1) * (2 * number + 1), 6)

  @doc """
  Calculate square of sum from 1 to a given end number.
  """
  @spec square_of_sum(pos_integer) :: pos_integer
  # def square_of_sum(number), do: Enum.sum(1..number) ** 2
